...
RUN apt update \
 && apt install -y libglew-dev libglfw3-dev libglm-dev libglu1-mesa-dev libxmu-dev

RUN apt update \
 && apt install -y cmake qtbase5-dev libxerces-c-dev libexpat1-dev
...
RUN mkdir -p /opt/cmake/src && curl -sL https://github.com/Kitware/CMake/archive/refs/tags/v3.31.6.tar.gz | tar -xz --strip-components 1 -C /opt/cmake/src \
...
RUN cmake --build $OPTICKS_PREFIX/build --parallel --target install
