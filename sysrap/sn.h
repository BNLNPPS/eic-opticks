...
struct SYSRAP_API sn
{
    ...
    **/
    ...
    static sn* Trap(double fz, double fy, double fx1, double fx2, double ft); 
...
inline sn* sn::Trap(double fz, double fy, double fx1, double fx2, double ft)  // static
{
    assert( fz > zero );
    assert( fy > zero );
    assert( fx1 > zero );
    ...
    nd->setPA( fz, fy, fx1, fx2, ft, zero );
    const double shift = ft * fy;
    const double x0min = -shift - fx1;
    const double x0max = -shift + fx1;
    const double x1min =  shift - fx2;
    const double x1max =  shift + fx2;

    const double xmin = std::min(x0min, x1min);
    const double xmax = std::max(x0max, x1max);
    nd->setBB( xmin, -fy, -fz,  xmax, +fy, +fz );
    return nd ;
}
...
inline void sn::setAABB_LeafFrame()
{
    ...
    {
    double fz, fy, fx1, fx2, ft, pad ;
    getParam_( fz, fy, fx1, fx2, ft, pad );
    assert( pad == 0.0 );

    assert( fz > 0.0 && fy > 0.0 && fx1 > 0.0 && fx2 > 0.0 );

    const double shift = ft * fy;

    const double x_min_y0 = -shift - fx1;   
    const double x_max_y0 = -shift + fx1;
    const double x_min_y1 =  shift - fx2;   
    const double x_max_y1 =  shift + fx2;

    const double xmin = std::min(x_min_y0, x_min_y1);
    const double xmax = std::max(x_max_y0, x_max_y1);

    setBB( xmin, -fy, -fz,  xmax, +fy, +fz );
    }
