...
inline QSIM_METHOD int qsim::propagate_at_boundary(unsigned& flag, RNG& rng, sctx& ctx, float theTransmittance ) const
{
    ...
    */
    ...
    // By calling curand_uniform four times, we’re consuming all four numbers
    // generated by a single Philox round, allowing the generator to move to the
    // next counter value cleanly
    curand_uniform(&rng);
    curand_uniform(&rng);
    curand_uniform(&rng);
    curand_uniform(&rng);
