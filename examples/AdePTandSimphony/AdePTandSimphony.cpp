...
class PhysicsList : public FTFP_BERT_AdePT {
  public:
    PhysicsList() : FTFP_BERT_AdePT(0) { RegisterPhysics(new G4OpticalPhysics); }
};
...
int main(int argc, char** argv)
{
    ...
    ui->ApplyCommand(buf);
    std::snprintf(buf, sizeof(buf), "/adept/setSeed %ld", seed);
    ui->ApplyCommand(buf);
    ui->ApplyCommand("/adept/setMillionsOfTrackSlots 2");
    ui->ApplyCommand("/adept/setCUDAStackLimit 8192");
    ui->ApplyCommand("/adept/setTrackInAllRegions true");
    ui->ApplyCommand("/adept/returnAllSteps true");
    ui->ApplyCommand("/adept/CallUserSteppingAction true");
