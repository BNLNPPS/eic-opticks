...
struct U4Solid
{
  ...
  private:
    ...
    static const G4VSolid *Right(
        const G4VSolid *node); // NB the G4VSolid returned might be a G4DisplacedSolid wrapping the G4VSolid
...
{
    std::stringstream ss;
    ss << std::setw(3) << lvid << "/" << depth << "/" << tag() << "/" << std::setw(3) << (root ? root->index() : -1)
       << "/" << std::setw(1) << level;
...
{
    std::stringstream ss;
    ss << brief() << " level " << level << " entityType " << std::setw(16) << (entityType ? entityType : "-")
       << " name " << (name ? name : "-");
    std::string str = ss.str();
    return str;
}
...
inline int U4Solid::Level(int level_, const char *name, const char *entityType) // static
{
    if (level_ > 0)
        return level_;
    if (IsFlaggedName(name) || IsFlaggedType(entityType))
        return 1;
    return -1;
}
...
{
    int type = _G4Other;
    if (strcmp(name, "G4Orb") == 0)
        type = _G4Orb;
    if (strcmp(name, "G4Sphere") == 0)
        type = _G4Sphere;
    if (strcmp(name, "G4Ellipsoid") == 0)
        type = _G4Ellipsoid;
    if (strcmp(name, "G4Box") == 0)
        type = _G4Box;
    if (strcmp(name, "G4Tubs") == 0)
        type = _G4Tubs;
    if (strcmp(name, "G4Trap") == 0)
        type = _G4Trap;
    if (strcmp(name, "G4Polycone") == 0)
        type = _G4Polycone;
    if (strcmp(name, "G4Cons") == 0)
        type = _G4Cons;
    if (strcmp(name, "G4Hype") == 0)
        type = _G4Hype;
    if (strcmp(name, "G4MultiUnion") == 0)
        type = _G4MultiUnion;
    if (strcmp(name, "G4Torus") == 0)
        type = _G4Torus;
    if (strcmp(name, "G4UnionSolid") == 0)
        type = _G4UnionSolid;
    if (strcmp(name, "G4SubtractionSolid") == 0)
        type = _G4SubtractionSolid;
    if (strcmp(name, "G4IntersectionSolid") == 0)
        type = _G4IntersectionSolid;
    if (strcmp(name, "G4DisplacedSolid") == 0)
        type = _G4DisplacedSolid;
    return type;
}
...
inline const char *U4Solid::Tag(int type) // static
{
    ...
    case _G4UnionSolid:
        tag = G4UnionSolid_;
        break;
    case _G4SubtractionSolid:
        tag = G4SubtractionSolid_;
        break;
    case _G4IntersectionSolid:
        tag = G4IntersectionSolid_;
        break;
...
inline const char *U4Solid::tag() const
{
    return Tag(type);
}
...
inline sn *U4Solid::Convert(const G4VSolid *solid, int lvid, int depth, int level) // static
{
    ...
    if (flagged_LVID)
        std::cout << "U4Solid::Convert"
                  << " solid.GetName[" << solid->GetName() << "]"
                  << " lvid " << lvid << " " << _U4Solid__IsFlaggedLVID << " : " << (flagged_LVID ? "YES" : "NO ")
...
inline U4Solid::U4Solid(const G4VSolid *solid_, int lvid_, int depth_, int level_)
    : solid(solid_), name(Name(solid_)), hint(Hint(name)), entityType(EType(solid)),
      level(Level(level_, name, entityType)), type(Type(entityType)), lvid(lvid_), depth(depth_), root(nullptr)
{
    init();
}
...
{
    if (level > 0)
        std::cerr << (depth == 0 ? "\n" : "") << "[U4Solid::init " << brief() << " " << (depth == 0 ? brief_KEY : "")
                  << " " << (depth == 0 ? name : "") << std::endl;
...
inline void U4Solid::init_Constituents()
{
    ...
    case _G4UnionSolid:
        ...
        break;
    case _G4IntersectionSolid:
        init_IntersectionSolid();
        break;
    case _G4SubtractionSolid:
        init_SubtractionSolid();
        break;
    case _G4DisplacedSolid:
        init_DisplacedSolid();
        break;
...
inline void U4Solid::init_Tree_Shrink()
{
    ...
    if (root != root0)
    {
        ...
        std::cerr << "U4Solid::init_Tree_Shrink CHANGED root with "
                     "sn::CreateSmallerTreeWithListNode_discontiguous/contiguous\n";
...
inline void U4Solid::init_Ellipsoid()
{
    ...
    if (level > 0)
        std::cerr << "U4Solid::init_Ellipsoid"
                  << " upper_cut " << upper_cut << " lower_cut " << lower_cut << " zcut1 " << zcut1 << " zcut2 "
                  << zcut2 << " zmin " << zmin << " zmax " << zmax << " sx " << sx << " sy " << sy << " sz " << sz
                  << " zslice " << (zslice ? "YES" : "NO") << std::endl;
    ...
    // see
    // notes/issues/unexpected_zsphere_miss_from_inside_for_rays_that_would_be_expected_to_intersect_close_to_apex.rst
    root->setXF(scale);
}
...
inline void U4Solid::init_MultiUnion()
{
    ...
    if (level > 0)
        std::cout << "[U4Solid::init_MultiUnion"
                  << " level " << level << " name " << name << " hint " << hint << " CSG::Name(hint) "
                  << CSG::Name(hint) << " type " << type << " CSG::Name(type) " << CSG::Name(type) << " sub_num "
                  << sub_num << "\n";
    ...
    for (unsigned i = 0; i < sub_num; i++)
    {
        ...
        if (level > 0)
            std::cout << "U4Solid::init_MultiUnion" << name << "\n" << stra<double>::Desc(xf) << "\n";
...
inline void U4Solid::init_Torus()
{
    ...
    if (level > 0)
        std::cout << " U4Solid::init_Torus "
                  << " rmin_mm " << rmin_mm << " rmax_mm " << rmax_mm << " rtor_mm " << rtor_mm << " startPhi_deg "
                  << startPhi_deg << " deltaPhi_deg " << deltaPhi_deg << "\n";
...
inline bool aaalmost_equal(G4double a, G4double b, G4double eps = 1e-12)
{
...
inline void U4Solid::init_Trap()
{
    ...
    assert(trap);
    std::cout << trap->GetZHalfLength() << std::endl;
    std::cout << trap->GetYHalfLength1() << std::endl;
    std::cout << trap->GetXHalfLength1() << std::endl;
    std::cout << trap->GetXHalfLength2() << std::endl;
    std::cout << trap->GetTanAlpha1() << std::endl;
    std::cout << trap->GetYHalfLength2() << std::endl;
    std::cout << trap->GetXHalfLength3() << std::endl;
    std::cout << trap->GetXHalfLength4() << std::endl;
    std::cout << "tanal" << trap->GetTanAlpha2() << std::endl;

    if (std::fabs(trap->GetYHalfLength1() - trap->GetYHalfLength2()) > 1e-5)
        assert(0);
    if (std::fabs(trap->GetXHalfLength1() - trap->GetXHalfLength3()) > 1e-5)
        assert(0);
    if (std::fabs(trap->GetXHalfLength2() - trap->GetXHalfLength4()) > 1e-5)
        assert(0);
    if (std::fabs(trap->GetTanAlpha2() - trap->GetTanAlpha1()) > 1e-5)
        assert(0);

    double fz = trap->GetZHalfLength() / CLHEP::mm;
    double fy = trap->GetYHalfLength1() / CLHEP::mm;
    double fx1 = trap->GetXHalfLength1() / CLHEP::mm;
...
inline void U4Solid::init_Polycone()
{
    ...
    if (level > 0)
        std::cerr << "U4Solid::init_Polycone"
...
{
    sn *outer = init_Cons_('O');
    assert(outer);
    sn *inner = init_Cons_('I');
    root = inner == nullptr ? outer : sn::Boolean(CSG_DIFFERENCE, outer, inner);

    if (level > 0)
        std::cerr << "U4Solid::init_Cons"
                  << " level " << level << desc();
}
...
inline OpticksCSG_t U4Solid::BooleanOperator(const G4BooleanSolid *solid) // static
{
    ...
    if (dynamic_cast<const G4IntersectionSolid *>(solid))
        _operator = CSG_INTERSECTION;
    else if (dynamic_cast<const G4SubtractionSolid *>(solid))
        _operator = CSG_DIFFERENCE;
    else if (dynamic_cast<const G4UnionSolid *>(solid))
        _operator = CSG_UNION;
...
inline const G4VSolid *U4Solid::Moved(G4RotationMatrix *rot, G4ThreeVector *tla, const G4VSolid *node) // static
{
    ...
    {
        if (rot)
            *rot = disp->GetFrameRotation();
        if (tla)
            *tla = disp->GetObjectTranslation();
...
inline void U4Solid::init_BooleanSolid()
{
    ...
    if (l->xform && level > 0)
        std::cout << "U4Solid::init_BooleanSolid "
                  << " observe transform on left node "
                  << " l.xform " << l->xform->desc() << std::endl;

    if (is_right_displaced)
        assert(r->xform && "expecting transform on right displaced node ");
